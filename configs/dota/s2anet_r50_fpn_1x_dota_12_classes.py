...
model = dict(
    ...
    fam_head=dict(
        ...
        anchor_scales=[4],  # This will produce {32², 64², 128², 256², 512²} with strides
...
data = dict(
    imgs_per_gpu=2,  # Adjust based on your GPU memory
    workers_per_gpu=2,
    train=dict(
        type=dataset_type,
        ann_file=data_root + 'annotations/train.pkl',
...
lr_config = dict(
    ...
    warmup_iters=500,
    warmup_ratio=1.0 / 3,
    step=[8, 11])  # LR drops at epochs 8 and 11
...
log_level = 'INFO'
load_from = None  # Start training from scratch for 12 classes
resume_from = None
workflow = [('train', 1)]
