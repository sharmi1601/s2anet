...
at::Tensor ARF_forward_cuda(const at::Tensor& weight,
  ...
  TORCH_CHECK(weight.is_cuda(), "input must be a CUDA tensor");
  TORCH_CHECK(indices.is_cuda(), "rois must be a CUDA tensor");
...
at::Tensor ARF_backward_cuda(const at::Tensor& indices,
  ...
  TORCH_CHECK(indices.is_cuda(), "input must be a CUDA tensor");
  TORCH_CHECK(gradOutput.is_cuda(), "rois must be a CUDA tensor");
