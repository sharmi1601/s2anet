...
std::tuple<at::Tensor, at::Tensor> RIE_forward_cpu(const at::Tensor& feature,
                                                   const uint8 nOrientation) {
  TORCH_CHECK(feature.ndimension() == 4, "only supports a batch of RIEs.");
  TORCH_CHECK(feature.size(2) == 1 && feature.size(3) == 1, "mH x mW should be 1x1.");
  TORCH_CHECK(!feature.is_cuda(), "input must be a CPU tensor");
...
at::Tensor RIE_backward_cpu(const at::Tensor& mainDirection,
                            ...
                            const uint8 nOrientation) {
  TORCH_CHECK(!mainDirection.is_cuda(), "input must be a CPU tensor");
  TORCH_CHECK(!gradOutput.is_cuda(), "rois must be a CPU tensor");
