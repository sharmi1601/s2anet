...
at::Tensor ARF_forward_cpu(const at::Tensor& weight,
  ...
  TORCH_CHECK(!weight.is_cuda(), "input must be a CPU tensor");
  TORCH_CHECK(!indices.is_cuda(), "rois must be a CPU tensor");
...
at::Tensor ARF_backward_cpu(const at::Tensor& indices,
  ...
  TORCH_CHECK(!indices.is_cuda(), "input must be a CPU tensor");
